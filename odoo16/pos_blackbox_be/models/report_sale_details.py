                ...
                report_update = {
                    ...
                    'Positive_discount_number': len(
                        self.env['pos.order'].search(
                            [('session_id', "=", session.id), ("amount_total", ">=", 0)]).filtered(
                        lambda o: o.lines.filtered(lambda l: l.discount > 0))),
                    'Negative_discount_number': len(
                        self.env['pos.order'].search(
                            [('session_id', "=", session.id), ("amount_total", "<", 0)]).filtered(
                        lambda o: o.lines.filtered(lambda l: l.discount > 0))),
                    'Positive_discount_amount': session.get_total_discount_positive_negative(True),
                    'Negative_discount_amount': session.get_total_discount_positive_negative(False),
                    'Correction_number': len(session.order_ids.filtered(lambda o: o.amount_total > 0).filtered(
                        lambda o: o.lines.filtered(lambda l: l.qty < 0))),
