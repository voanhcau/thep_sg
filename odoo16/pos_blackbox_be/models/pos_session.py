...
class pos_session(models.Model):
    ...
    def _pos_data_process(self, loaded_data):
        ...
        loaded_data['product_product_work_in'] = self.env.ref('pos_blackbox_be.product_product_work_in').id
        loaded_data['product_product_work_out'] = self.env.ref('pos_blackbox_be.product_product_work_out').id
    ...
    def _get_total_correction(self):
        total_corrections = 0
        for order in self.order_ids.filtered(lambda o: o.amount_total > 0):
            for line in order.lines:
    ...
    def get_total_discount_positive_negative(self, positive):
        ...
        orderlines = self.env['pos.order.line'].search(
            [('order_id', 'in', self.order_ids.ids), ('price_subtotal_incl', '>=' if positive else '<', 0),
             ('discount', '>', 0)])
