...
def analyze_before():
    ...
    try:
        ...
        recent_pos = models.execute_kw(db, uid, password,
            ...
            [recent_po_domain],
            {'fields': ['id', 'name', 'state', 'date_approve', 'partner_id']}
        )
        ...
        for po in recent_pos:
            ...
            po_state_count[state] = po_state_count.get(state, 0) + 1
        ...
        print("  - Purchase Orders by state:")
        for state, count in po_state_count.items():
        ...
        return {
            ...
            'po_state_count': po_state_count
        }
        
    except Exception as e:
        print("ERROR in analysis: %s" % str(e))
        return {'so_count': 0, 'po_count': 0, 'problematic_count': 0, 'recent_count': 0, 'po_state_count': {}}
        ...
        for so_id in so_ids:
            try:
                models.execute_kw(db, uid, password,
                    "sale.order", "compute_invoice_status",
                    [[so_id]]
                )
                success_count += 1
...
def analyze_after():
    ...
    try:
        ...
        # Count POs by state after fix
        po_state_count_after = {}
        for po in recent_pos:
            state = po.get('state', 'unknown')
            po_state_count_after[state] = po_state_count_after.get(state, 0) + 1
...
def main():
    ...
    try:
        ...
        # Auto-confirm - removed user prompt
